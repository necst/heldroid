...
public class MainServer implements Runnable {
    ...
    public MainServer(File uploadDirectory) throws ParserConfigurationException {
        ...
        if (!samplesCliScript.exists())
            throw new RuntimeException("samples_cli.py not found in " + uploadDirectory.getAbsolutePath());
    ...
    private class ScanHandler extends BaseHandler implements HttpHandler {
        ...
        public void handle(HttpExchange t) throws IOException {
            File requestFile = File.createTempFile("upload-", ".apk", MainServer.this.uploadDirectory);
        ...
        protected File saveRequestFile(HttpExchange exchange, File resultFile) throws IOException {
    ...
    private class PcapScanHandler extends ScanHandler implements HttpHandler {
        @Override
        public void handle(HttpExchange t) throws IOException {
            File requestFile = File.createTempFile("upload-", ".pcap", MainServer.this.uploadDirectory);
            this.saveRequestFile(t, requestFile);
            ...
            if (response == null) {
                response = MainServer.this.buildResponseFromPcapScan(requestFile);
                MainServer.this.saveResponseByHash(hash, response);
            }
